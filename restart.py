...
print(f"Restarting Remote Board with IP {target}")
...
if ip_re.match(target):
    try:
        ...
        print(f"Reverse DNS lookup failed for {target}")
    if host:
        matches = re.match(node_name_re, host).groups()
        netbox_device = matches[0] + "-" + matches[1]
        target = host

print(f"Resolved to hostname {host}")
      
print(f"Get Manufacturer from Netbox for device {netbox_device} ...")

netbox_url = "https://netbox.global.cloud.sap"
netbox_token = os.getenv("NETBOX_API_KEY")
netbox_params = {'q': netbox_device}
...
if response.status_code == 202 or response.status_code == 204:
    print("Remote Board restart initiated successfully")
else:
    print(f"Error resetting Remote Board: {response.status_code}")
    print(response.text)
